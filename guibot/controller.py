...
class Controller(LocalConfig):
    ...
    def _region_from_args(self, *args) -> list[int, str]:
        if len(args) == 4:
            xpos = args[0]
            ypos = args[1]
            width = args[2]
    ...
    def capture_screen(self, *args: region.Region | None) -> target.Image:
        """
    ...
    def mouse_move(self, location: location.Location, smooth: bool = True) -> None:
        """
...
class PyAutoGUIController(Controller):
    ...
    def configure_backend(self, backendstr = None, category: str = "pyautogui",
                            reset: bool = False) -> None:
        """
        Custom implementation of the base method.
