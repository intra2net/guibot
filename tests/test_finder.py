...
class FinderTest(unittest.TestCase):
    ...
    def _get_matches_in(self, pattern: Pattern, dumps: int) -> int:
        return [match.group(0) for d in dumps for match in [re.search(pattern, d)] if match]

    def _verify_and_get_dumps(self, count: int, index: int = 1, multistep: bool = False) -> int:
    ...
    def _verify_dumped_images(self, needle_name: str, haystack_name: str, dumps: int, backend: str) -> None:
    ...
    def _verify_single_hotmap(self, dumps: int, backend: str) -> None:
